...
;(() => {
  ...
  function smartShuffle(conf, above, left) {
    ...
    if (above && rand() < conf.randomness)
      frndarr.splice(frndarr.indexOf(above), 1)
    if (left && rand() < conf.randomness)
      frndarr.splice(frndarr.indexOf(left), 1)
  ...
  function genTriProps(conf, above, left) {
    ...
    return {
      colorSeq: r_seq,
      speed:
        Math.floor(rand() * (conf.maxSpeed - conf.minSpeed)) + conf.minSpeed,
      points: '',
    }
  }
