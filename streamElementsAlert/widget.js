...
;(async () => {
  const {
    ...
    window.addEventListener('onWidgetLoad', callback)
  )
  ...
  async function queryGiphy(query) {
    const gifs = await (
      await fetch(
        `https://api.giphy.com/v1/gifs/search?api_key=${key}&q=${query}&limit=1&rating=pg-13`
      )
    ).json()
    console.log(gifs)
  ...
  async function processEvent(event) {
    ...
    const action = {
      ...
      subscriber:
        includeSubs && !event.gifted && `just donated ${amount} subs!`,
    ...
    createBouncingImage({
      ...
      textElem: `
					...
					${
            message
              ? `<p style="font-size:80%; text-shadow: inherit;">${message}</p>`
              : ''
          }
				`,
    })
